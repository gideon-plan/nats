...
proc open_nats*(host: string, port: int = 4222, verbose: bool = false,
  ...
  if info.val.kind != NatsMsgKind.Info: raise newException(NatsError, "expected INFO")
  result.server_info = info.val.info_json
  # Send CONNECT
  let connect_json = "{\"verbose\":" & $verbose & ",\"name\":\"" & name & "\",\"protocol\":1}"
  send_raw(result, encode(NatsMsg(kind: NatsMsgKind.Connect, connect_json: connect_json)))
  if verbose:
    let ok_line = read_line(result.sock)
    let ok = decode_line(ok_line)
    if ok.is_bad or ok.val.kind != NatsMsgKind.Ok:
...
proc recv_msg*(conn: NatsConn): NatsMsg {.raises: [NatsError].} =
  ...
  result = parsed.val
  if result.kind == NatsMsgKind.Msg:
