...
suite "proto encode":
  test "encode PING":
    check encode(NatsMsg(kind: NatsMsgKind.Ping)) == "PING\r\n"

  test "encode PONG":
    check encode(NatsMsg(kind: NatsMsgKind.Pong)) == "PONG\r\n"

  test "encode PUB":
    let msg = NatsMsg(kind: NatsMsgKind.Pub, pub_subject: "test", pub_payload: "hello")
    let enc = encode(msg)
  ...
  test "encode PUB with reply":
    let msg = NatsMsg(kind: NatsMsgKind.Pub, pub_subject: "test", pub_reply: "inbox.1",
                      pub_payload: "hi")
    let enc = encode(msg)
    check enc.contains("inbox.1")

  test "encode SUB":
    let msg = NatsMsg(kind: NatsMsgKind.Sub, sub_subject: "events.>", sub_sid: "1")
    check encode(msg) == "SUB events.> 1\r\n"

  test "encode SUB with queue":
    let msg = NatsMsg(kind: NatsMsgKind.Sub, sub_subject: "work", sub_queue: "workers",
                      sub_sid: "2")
    check encode(msg) == "SUB work workers 2\r\n"

  test "encode UNSUB":
    check encode(NatsMsg(kind: NatsMsgKind.Unsub, unsub_sid: "1")) == "UNSUB 1\r\n"

  test "encode UNSUB with max":
    check encode(NatsMsg(kind: NatsMsgKind.Unsub, unsub_sid: "1", unsub_max: 5)) == "UNSUB 1 5\r\n"

  test "encode CONNECT":
    let msg = NatsMsg(kind: NatsMsgKind.Connect, connect_json: "{\"verbose\":false}")
